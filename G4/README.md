...
사람의 수와 파티의 수가 주어지고, 각 파티에 참석하는 사람들이 주어질 떄, 거짓말임을 아는 사람들이 참석하는 파티와, 해당 파티에 같이 참석하는 사람들이 참석하는 파티는 참석할 수 없다고 할 때, 참석이 가능한 파티의 수를 구하는 문제

각 파티에 참석하는 모든 사람들을 서로 이어주는 그래프를 만들었다.

위 그래프는 인접 리스트를 사용하여 표현하였다.

그리고 맨처음주어지는 거짓말을 아는 사람들을 시작으로 bfs를 통해, 연결된 모든 사람들을 확인하였다.

그리고 위 인원이 참석하는 파티는 배제하면서, 남은 파티의 수를 구하는 것으로 풀이하였다.
...
CCTV의 위치와 벽의 위치가 주어지고, CCTV 종류에 따라 감시할 수 있는 방향이 다르고, 직각으로 회전시킬 수 있다고 할 때, 사각지대가 최소가 될 때의 사각지대의 수를 구하는 문제

CCTV 5번을 제외한 나머지의 위치를 저장하는 List를 만들어 저장하고, CCTV 5번의 경우, 네방향으로 0을 -1로 칠한다.

CCTV 위치를 저장한 List의 길이를 기반으로, 0~3을 모두 가질 수 있도록 dfs를 진행한다.

각 dfs의 끝지점 마다, 기존의 office를 깊은 복사하여, 사각지대를 그려보고 그 수를 세어 가장 작은 수를 출력하였다.
