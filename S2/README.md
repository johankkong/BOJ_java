...
M x N 크기의 밭에 배추가 심어진 위치가 주어지고, 상하좌우로 붙어있는 배추를 하나의 그룹으로 생각할 때, 몇개의 그룹이 만들어지는 지 구하는 문제

배추의 위치를 입력받으면서 배추가 심어진 위치는 true, 빈칸은 false로 입력 받았다.

그 후, 모든 칸을 탐색하면서, true를 만나면, 그룹 수를 하나 늘리고, 해당 칸과 연결된 칸들을 모두 false로 바꿔주는 작업을 하였다.

이 과정에서 재귀를 통해 모든 방향에 대해 탐색이 가능하도록 하였다.

## 1260 - DFS와 BFS

22.10.29 풀이

주어진 간선들의 정보를 입력받고 해당 그래프를 깊이 우선 탐색으로 탐색했을 때의 결과와 너비 우선 탐색으로 탐색했을 때의 결과를 출력하는 문제

DFS는 stack을 이용하고 BFS는 queue를 이용한다.

DFS는 방문처리를 꺼낼때 해도 상관없지만, BFS는 방문처리를 넣을 때 해야 중복을 방지할 수 있다. 

## 1541 - 잃어버린 괄호

24.10.12 풀이

주어진 +와 - 연산만 존재하는 수식에서 `()`를 원하는 곳에 넣어 가장 작은 수를 만드는 문제

`그리디` 형식으로 풀이 하였다.

`-` 연산하기 전까진 모두 수를 더하고, 한번이라도 `-`가 등장하면 그 뒤 수는 모두 빼는 방식으로 문제를 풀이

숫자가 나올 때마다 `StringBuiler`에 저장하고 연산자를 만나면 `Integer.parseInt`메소드로 숫자로 만들어서 연산을 진행
...
N개의 숫자가 주어지고, 그 안에서 연속된 수를 뽑아서 그 합을 구할 때, 최대값을 구하는 문제

DP를 이용하여 풀이하였다.

앞에서부터 DP값이 양수면 그 다음 값을 더해서 DP에 저장하고, DP값이 음수라면, 더하지 않고 그 다음 값만 DP배열에 저장해주는 방식으로 DP배열을 채웠다.

모두 DP 배열을 채운뒤 그 중 최대값을 출력하는 방식으로 풀이하였다. 

## 1927 - 최소 힙

24.10.12 풀이

최소 힙을 이용하여 0이 나오면 하나씩 `poll`을 하여 출력하는 문제

java에서 기본적으로 제공하는 `priorityQueue`를 이용하여 풀이

힙을 직접 구현해야 했다면 어려웠을 것..
...
N x N 배열에 서로 다른 수가 채워져있다고 할 때, 배열 안에서 모든 수는 자신의 바로 아래 칸보다 작은 수가 들어있다. 이 때, N번째로 큰 수를 구하는 문제

맨 아래 행부터 탐색하면서 다섯번째 수를 구하고, 다음 행을 탐색할 때, 그 수보다 작은 열을 더이상 탐색하지 않아도 된다.

각 열에 대해 탐색할 지 말 지 저장할 boolean배열을 만들었다.

맨 아래 행부터 탐색하면서, 현재 수보다 큰 수만 list에 저장하고 그렇지 않은 경우, 그 행을 boolean배열을 통해 제외해주었다.

탐색해 나가다가, 모든 행에 대해서 boolean배열이 true가 되면 더이상, 탐색하지 않고 중단한 뒤 그 당시의 5번째 수를 출력하는 것으로 풀이하였다. 
...
N x M 크기의 배열에 육지와 바다의 위치가 주어진다고 할 때, 8방향으로 인접한 육지는 하나의 섬으로 간주한다. 배열에는 총 몇 개의 섬이 있는 지 출력하는 문제

bfs를 이용하여 풀이하였다. 섬이면서 visited배열에서 방문한 적이 없는 육지 부분에서 bfs를 시작할 때마다 그 갯수를 세었다.
...
수열이 주어졌을 때, 증가하는 부분 수열 중에서 그 합이 가장 큰 부분 수열을 구해 그 합을 구하는 문제

DP를 이용하여 풀이하였다.

앞에서 부터 채워나가면서, 현재 값보다 작은 값들 중에서 가장 큰 dp값에 현재 값을 더하여, dp배열을 채워나갔다.

dp배열을 모두 채운 뒤에 가장 큰 값이 가장 큰 증가 부분 수열의 합이다.

## 11084 - 이동하기

22.10.06 풀이

왼쪽위에서 오른쪽아래로 이동하면서, 해당 칸에 있는 수를 더해나간다고 할때, 그 최댓값을 출력하는 문제

DP를 통해 풀이하였다.

DP배열은 현재칸을 채울 때, 왼쪽, 왼쪽위, 위쪽 세칸 중 가장 큰 값에 현재 칸의 값을 더해나가는 것으로 풀이하였다.
...
N x M 범위에 땅의 높이가 주어지고, 현재 가지고 있는 블럭의 수가 주어진다고 할 때, 블럭을 빼는 건 2초, 블럭을 놓는 건 1초라고 할 때, 최소 시간으로 땅을 평탄하게 만드는 때, 최소 시간과 그 시간에 만들 수 있는 최고 높이의 땅을 출력하는 문제

가장 작은 땅에서 가장 높은 땅의 높이까지 적용하면서 그 높이로 평탄화하는데 걸리는 시간과 필요한 블럭의 수를 계산한다.

필요한 블럭의 수가 현재 가진 블럭보다 많다면 그 높이는 넘어간다.

가장 적게 걸리는 시간에서 가장 높은 높이를 찾아 출력한다.
