...
틱택토 게임의 게임판의 상태를 입력받아 해당 상태가 틱택토 게임의 마지막 상태인지 확인하는 문제

X와 O의 갯수를 비교하여 O가 더 많거나 2이상 차이가 나면 불가능한 상태이다.

3을 완성한 줄 수가 2개 이상이거나, 0개이면 기본적으로는 마지막 상태가 불가능하지만,

X가 5개 이면서 X가 3을 2개 만들었다면 가능한 상태이다.

0개에서도 O가 4개 X가 5개인 경우엔 가능하다.

1개만 있는 경우엔, 기본적으로는 가능한 상태이지만,

O가 3개를 만들었는데, X의 갯수가 더 많다면 불가능한 상태이고

X가 3개를 만들었는데, O와 X의 갯수가 같다면 불가능한 상태이다.
...
4개의 톱니바퀴의 각 8개의 이빨은 N극 혹은 S극을 가질 수 있다. 한 개의 톱니바퀴는 선정하여 시계방향 혹은 반시계방향으로 돌릴 수 있는데 그 과정에서 맞닿아있는 좌우의 톱니바퀴의 극이 다르면 연쇄적으로 돌아간다.

4개의 톱니바퀴 이빨의 정보가 주어지고, 몇번 톱니바퀴를 어느 방향으로 돌릴 지 주어진다고 할 때, 최종 상태를 구하는 문제

톱니바퀴 클래스를 만들어, 각 이빨의 극 정보를 배열로 저장하였다. 그리고 왼쪽과 오른쪽에 해당하는 자리의 인덱스를 추적하였다. 
톱니바퀴가 한 칸 돌아갈 때마다, 인덱스가 움직이는 방식이다.

돌려야하는 톱니바퀴가 주어지면, 좌우로 각각 돌려야하는 톱니바퀴를 결정하고, 각자 돌려야하는 방향으로 한번에 돌리는 방식을 채택하였다.
...
N x N 좌표의 각 칸을 나라라고 생각하고, 인구수가 저장된다고 할 떄, 매일 일정 범위이내의 인구수 차이를 보이는 국경선을 개방하고 국경선이 개방된 나라들끼리 인구수를 동일하게 나눈다고 할 떄, 며칠동안 인구 이동이 일어나는 지 출력하는 문제

각 나라에 번호를 부여할  N x N 크기의 배열을 준비하고

위에서 부여한 번호의 속하는 나라의 수와 인구수합을 저장할 2501크기의 배열을 준비한다.

그 뒤로는 BFS를 통해 국경선을 개방하는 나라들을 각각 번호를 매기고 인구수를 더해나간다.

그 뒤엔 번호와 인구수에 따라 인구수를 분배하고 날짜를 1 더한다.

위 과정을 오른쪽 아래칸의 번호가 N * N이 될 떄까지 반복한 뒤, 끝나면 날짜를 출력하는 것으로 풀이하였다.
