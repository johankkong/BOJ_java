...
지도의 크기 N과 경사로의 길이 L이 주어진다고 할 때, 가로, 세로 방향으로 통행할 수 있는 길의 수를 출력하는 문제

각 행과 열에 대해 통행할 수 있는 길인지 판단하는 메소드를 적용하여 풀이하였다.

통행할 수 있는 길이지는 순차적으로 조건을 부여하여 구현하였다.

2이상 차이가 나는 경우, 통행할 수 없음

차이가 1이면서 내리막인 경우, L만큼 평평한 땅이 이어지는 가

차이가 1이면서 오르막인 경우, L만큼 평평한 땅이 이어지면서, 이미 경사로가 놓여지지 않았는가

위 세가지 조건을 바탕으로 풀이하였다.
