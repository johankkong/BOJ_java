...
N개의 문자열을 입력받고 난 뒤, 숫자가 주어지면 해당 번째의 문자열을, 문자열이 주어지면 몇 번째 문자열인지 출력하는 문제

숫자로 인덱싱할 때 빠른 배열, 문자열을 통해 숫자를 받는 것이 빠른 Map. 둘 중 하나만 이용하여 풀이해보려 했으나 시간초과가 발생하였다.

따라서 두가지 모두를 사용하여 숫자를 이용할 땐 배열을, 문자열이 주어지면 Map을 이용하여 출력하였다.

## 1764 - 듣보잡

22.09.30 풀이

N개의 문자열과 M개의 문자열을 입력받아 양쪽 모두에 있는 문자열의 수를 출력하고 그 밑에 해당 문자열들을 사전 순으로 출력하는 문제

한쪽의 문자열들을 TreeMap에 저장하여 다른 한쪽에서 조회하는 것을 수월하도록 하였고,

다른 한 쪽의 문자열들은 PriorityQueue에 저장하여 사전 순으로 나올 수 있게 하였다.

모든 문자열을 각각의 자료 구조에 넣고, PQ에서 하나씩 꺼내면서 TreeMap에 있는 지 확인한다.

있는 경우, 갯수를 하나 추가해 주고 StringBuilder에 추가한다.
...
ATM에서 현금을 인출하는 속도가 주어지고, 줄을 세운다고 할 때, 모든 사람의 대기시간과 인출하는 시간을 더한 시간이 최소가 될 때의 합을 구하는 문제

앞쪽에 배치하는 사람이 걸리는 시간은 뒤에 사람들의 대기시간에 들어가기 때문에 시간이 적게 걸리는 사람들 앞쪽에 배치하여야한다.

따라서, 배열에 각 시간을 저장하고, 오름차순으로 정렬한 뒤, 앞쪽부터 N, N - 1, .. 을 각각 곱하여 더하는 것으로 풀이하였다.
