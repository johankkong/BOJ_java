...
자연수 N을 입력받아 2 x N 크기를 1 x 2크기의 타일로 채울 수 있는 경우의 수를 구하는 문제

간단하게 DP로 i-1항과 i-2항을 더해 i항을 채워나가면 된다.

다만, 입력받는 숫자가 커짐에 따라 기하급수적으로 그 수가 늘기 때문에, 더하기 연산을 할 때마다 10007로 나눠주는 연산을 해야한다.

## 11727 - 2 x N 타일링 2

22.10.06 풀이

자연수 N을 입력받아 2 x N 크기를 1 x 2크기의 타일과 2 x 2크기의 타일로 채울 수 있는 경우의 수를 구하는 문제

같은 방식으로 i-1항과 i-2항을 더해 i항을 채워나가면 된다.

하지만, i-2항에서 2 x 1 타일을 두개 쓴 경우와  2 x 2 타일 하나를 쓰는 경우로 나눠지기 때문에, i - 2항에 2를 곱해서 더해줘야한다.
...
해당 날짜부터 며칠동안 일하면, 받을 수 있는 지 주어져있다고 할 때, 일정 기간동안 최대한 얻을 수 있는 수익을 구하는 문제

DP로 풀이하였다.

가장 뒤에서 부터, 앞 방향으로 DP 배열을 채워나갔다.

해당 날짜에서 시작하는 상담이 끝나는 날 이후로 가장 큰 DP값에 자신의 DP을 더해가는 방향으로 채워나갔다.

배열을 채운 뒤, DP배열에서 가장 큰 값을 출력하였다. 

## 15650 - N과 M (2)

22.11.01 풀이

자연수 N과 M이 주어지고, 1부터 N까지의 숫자 중에서 M개를 뽑아 만들 수 있는 순열을 모두 출력하는 문제

오름차순으로 순열을 구성해야한다고 문제에서 주어졌기 때문에, 이 문제는 순열이 아니라 조합이다.

N개의 자연수 중에서 M개를 뽑아서 오름차순으로 나열한 결과를 나열하여 풀이하였다.

> 보완할 점

조합을 뽑는 과정에서 이미 오름차순으로 뽑기 때문에, 오름차순 정렬과정이 불필요했다.

## 15652 - N과 M (4)

22.11.02 풀이

1부터 N까지의 자연수 중에서 M개의 수를 중복을 허용하여 비내림차순 순열을 모두 출력하는 문제

순열이라고 명시되어있지만, 순서가 정해져 있기때문에, 중복을 허용한 조합이다.

한번 뽑을 때마다, 해당 수를 포함하여 더 큰 수만 그 다음에 뽑을 수 있도록하였다.

## 15654 - N과 M (5)

22.11.03 풀이

N개의 자연수를 입력받아 M개를 뽑아 가능한 모든 수열을 사전순으로 출력하는 문제

가장 먼저, N개의 자연수를 입력받아, 오름차순으로 정렬하였다.

그리고 작은 것부터 뽑아 수열을 만드는 방식으로 풀이하였다.
